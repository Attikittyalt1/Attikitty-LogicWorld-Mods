...
public class Handler2D<T>
    where T : ILinkable<T>
{
    public LinkableContainer2D<T> Linkable { get; init; }
    public Func<ComponentAddress> GetAddress { get; init; }

    private ComponentAddress? _trackerKey;
    private bool _isTracked = false;
    private readonly List<PackageManager2D<T>> _currentManagers = [];
    ...
    public void StartTracking(IEnumerable<PackageManager2D<T>> packageManagers)
    {
        if (_isTracked)
        {
    ...
    public bool TryStartTracking(IEnumerable<PackageManager2D<T>> packageManagers)
    {
        if (!IsBeingTracked())
        {
